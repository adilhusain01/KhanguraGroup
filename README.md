...
1. Set preview and production environment variables in the hosting provider; use distinct Atlas databases and Cloudinary folders.
...
4. Set `VITE_TURNSTILE_SITE_KEY` alongside `TURNSTILE_SECRET_KEY`, configure the Meta webhook as `/api/webhooks/whatsapp`, and set `CRON_SECRET` in the hosting provider. Add the same secret plus `CRON_TARGET_URL` (for example, `https://www.example.com`) to GitHub repository Actions secrets. The scheduled maintenance workflow runs WhatsApp retries every 15 minutes and retention daily at 04:00 UTC.
